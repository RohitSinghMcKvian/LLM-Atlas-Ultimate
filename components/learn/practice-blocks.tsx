...
export function PredictBlock({ block }: { block: PredictSpec }) {
  ...
  return (
    <div className="not-prose overflow-hidden rounded-2xl border border-accent/30 bg-surface/60">
      <div className="flex items-center gap-2 border-b border-border bg-accent/[0.07] px-3.5 py-2.5">
        <span className="grid size-5 place-items-center rounded bg-accent/20 text-accent">
      ...
      <div className="p-3.5">
        ...
        {hasOptions ? (
                ...
                >
                  <span
                    className={cn(
                      ...
                      choice === i ? "border-accent" : "border-border-strong",
                    )}
                  >
                    {choice === i && (
          ...
          <>
            ...
            <textarea
              ...
              className="mt-3 w-full resize-y rounded-xl border border-border bg-surface-2/50 p-2.5 text-sm leading-relaxed outline-none transition-colors focus:border-accent/50 disabled:opacity-70"
            />
          </>
        )}
        ...
        <AnimatePresence initial={false}>
          {revealed && (
            ...
            >
              <div className="rounded-xl border border-accent/30 bg-action/10 p-3">
                <p className="mb-1 text-2xs font-semibold uppercase tracking-wider text-accent">
...
export function SortBlock({ block }: { block: SortSpec }) {
  ...
  return (
    <div className="not-prose overflow-hidden rounded-2xl border border-action/30 bg-surface/60">
      <div className="flex items-center gap-2 border-b border-border bg-action/[0.07] px-3.5 py-2.5">
        <span className="grid size-5 place-items-center rounded bg-action/20 text-action">
            ...
            return (
              <motion.li
                ...
                className={cn(
                  ...
                  !checked && "border-action/35 bg-action/[0.05]",
                )}
              >
                <span className="grid size-5 shrink-0 place-items-center rounded-full bg-surface-3 font-mono text-2xs font-bold">
                  {i + 1}
                </span>
            ...
            {remaining.map((item) => (
              <li key={item.id}>
                <button
                  ...
                  className="rounded-lg border border-border bg-surface-2/60 px-2.5 py-1.5 text-left text-xs transition-colors hover:border-action/50 hover:bg-action/[0.07]"
                >
                  {item.text}
                </button>
              </li>
