...
| **News** | Live AI news from ~40 first-party, research and press feeds. Keeps itself current with no API key and no sync button, de-duplicated into cross-publisher clusters, scored for provenance (`verified` / `corroborated` / `reported`), illustrated from each publisher's own `og:image`, and always linked to the original. Optional daily brief by Web Push |
